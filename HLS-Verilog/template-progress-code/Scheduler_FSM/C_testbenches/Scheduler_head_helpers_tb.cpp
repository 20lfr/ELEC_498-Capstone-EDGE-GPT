...
// Pretty format booleans: 1 or '-'
static inline const char* bstr(bool v) { return v ? "1" : "-"; }
...
void test_run_head_group() {
    ...
    auto fmt_array = [&](const bool* arr) {
        std::ostringstream oss;
        oss << "[";
        for (int h = 0; h < heads_in_group; ++h) {
            oss << (arr[h] ? "1" : "-");
            if (h != heads_in_group - 1) oss << ", ";
        }
        oss << "]";
        return oss.str();
    };
    ...
    for (int cycle = 0; cycle < 128 && !group_done; ++cycle) {
        ...
        std::cout << std::left
                  << std::setw(6) << cycle << "|"
                  << std::setw(5) << bstr(reset_resources) << "|"
                  << std::setw(11) << bstr(reset_resources) << "|"
                  << std::setw(8) << bstr(compute_ready) << "|"
                  << std::setw(10) << bstr(compute_done) << "|"
                  << std::setw(10) << bstr(compute_start) << "|"
                  << std::setw(8) << compute_str(compute_op) << "|"
                  << std::setw(10) << bstr(group_done) << "|"
                  << std::setw(12) << bstr(res.comp_busy) << "|"
                  << std::setw(12) << fmt_array(res.request_compute) << "|"
                  << std::setw(12) << fmt_array(res.grant_compute) << "|"
                  << std::setw(8)  << bstr(res.currently_granting);
        for (int h = 0; h < heads_in_group; ++h) {
