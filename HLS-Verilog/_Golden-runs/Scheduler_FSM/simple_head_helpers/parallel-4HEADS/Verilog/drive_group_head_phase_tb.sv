...
module drive_group_head_phase_tb;
  ...
  logic [65:0] head_ctx_ref_2_i;
  logic [65:0] head_ctx_ref_2_o;
  logic        head_ctx_ref_2_o_ap_vld;
  logic [65:0] head_ctx_ref_3_i;
  logic [65:0] head_ctx_ref_3_o;
  ...
  // ========================================================================
  always_comb begin
    head_ctx_t temp0, temp1, temp2, temp3;
    int group_base;
    ...
    // Head 2 lane
    temp2 = '0;
    if (group_base + 2 < HEADS_TOTAL) begin
      temp2 = head_ctx_shadow[group_base + 2];
      temp2.compute_ready = ext_compute_ready[group_base + 2];
      temp2.compute_done  = ext_compute_done[group_base + 2];
    end
    head_ctx_ref_2_i = temp2;

    // Head 3 lane
    temp3 = '0;
    if (group_base + 3 < HEADS_TOTAL) begin
      temp3 = head_ctx_shadow[group_base + 3];
      temp3.compute_ready = ext_compute_ready[group_base + 3];
      temp3.compute_done  = ext_compute_done[group_base + 3];
    end
    head_ctx_ref_3_i = temp3;
  ...
  always_ff @(posedge clk) begin
    if (ap_rst) begin
      ...
      if (head_ctx_ref_0_o_ap_vld && gb < HEADS_TOTAL) begin
        head_ctx_shadow[gb] <= head_ctx_ref_0_o;
      end
      if (head_ctx_ref_1_o_ap_vld && (gb + 1) < HEADS_TOTAL) begin
        head_ctx_shadow[gb + 1] <= head_ctx_ref_1_o;
      end
      if (head_ctx_ref_2_o_ap_vld && (gb + 2) < HEADS_TOTAL) begin
        head_ctx_shadow[gb + 2] <= head_ctx_ref_2_o;
      end
      if (head_ctx_ref_3_o_ap_vld && (gb + 3) < HEADS_TOTAL) begin
        head_ctx_shadow[gb + 3] <= head_ctx_ref_3_o;
      end
    end
  end
    ...
    for (genvar h = 0; h < HEADS_TOTAL; h++) begin : COMPUTE_MODEL
      ...
      always_ff @(posedge clk) begin
        if (ap_rst) begin
          ...
          end else if (h == base_head_idx + 1 && head_ctx_ref_1_o_ap_vld) begin
            compute_start_now = head_ctx_ref_1_o[42];
          end else if (h == base_head_idx + 2 && head_ctx_ref_2_o_ap_vld) begin
            compute_start_now = head_ctx_ref_2_o[42];
          end else if (h == base_head_idx + 3 && head_ctx_ref_3_o_ap_vld) begin
            compute_start_now = head_ctx_ref_3_o[42];
          end else begin
  ...
  always_ff @(posedge clk) begin
    if (ap_rst) begin
      ...
      gb = base_head_idx;
      $display("%0d %3d ret:%b | %-8s  %0d   %0d    %0d   %-5s  %0d   %0d   %0d   %0d   %0d   %0d  vld:%0d | %-8s  %0d   %0d    %0d   %-5s  %0d   %0d   %0d   %0d   %0d   %0d  vld:%0d | %-8s  %0d   %0d    %0d   %-5s  %0d   %0d   %0d   %0d   %0d   %0d  vld:%0d | %-8s  %0d   %0d    %0d   %-5s  %0d   %0d   %0d   %0d   %0d   %0d  vld:%0d |",
               cycle_count,
               group_idx,
               ap_return,
               (gb < HEADS_TOTAL) ? phase_name(head_ctx_shadow[gb].phase) : "?",
               (gb < HEADS_TOTAL) ? ext_compute_ready[gb] : 0,
               (gb < HEADS_TOTAL) ? ext_compute_done[gb] : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].compute_start : 0,
               (gb < HEADS_TOTAL) ? op_name(head_ctx_shadow[gb].compute_op) : "?",
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].q_started : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].q_compute_done : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].k_started : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].k_compute_done : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].v_started : 0,
               (gb < HEADS_TOTAL) ? head_ctx_shadow[gb].v_compute_done : 0,
               head_ctx_ref_0_o_ap_vld,
               (gb + 1 < HEADS_TOTAL) ? phase_name(head_ctx_shadow[gb+1].phase) : "?",
               (gb + 1 < HEADS_TOTAL) ? ext_compute_ready[gb+1] : 0,
               (gb + 1 < HEADS_TOTAL) ? ext_compute_done[gb+1] : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].compute_start : 0,
               (gb + 1 < HEADS_TOTAL) ? op_name(head_ctx_shadow[gb+1].compute_op) : "?",
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].q_started : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].q_compute_done : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].k_started : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].k_compute_done : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].v_started : 0,
               (gb + 1 < HEADS_TOTAL) ? head_ctx_shadow[gb+1].v_compute_done : 0,
               head_ctx_ref_1_o_ap_vld,
               (gb + 2 < HEADS_TOTAL) ? phase_name(head_ctx_shadow[gb+2].phase) : "?",
               (gb + 2 < HEADS_TOTAL) ? ext_compute_ready[gb+2] : 0,
               (gb + 2 < HEADS_TOTAL) ? ext_compute_done[gb+2] : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].compute_start : 0,
               (gb + 2 < HEADS_TOTAL) ? op_name(head_ctx_shadow[gb+2].compute_op) : "?",
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].q_started : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].q_compute_done : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].k_started : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].k_compute_done : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].v_started : 0,
               (gb + 2 < HEADS_TOTAL) ? head_ctx_shadow[gb+2].v_compute_done : 0,
               head_ctx_ref_2_o_ap_vld,
               (gb + 3 < HEADS_TOTAL) ? phase_name(head_ctx_shadow[gb+3].phase) : "?",
               (gb + 3 < HEADS_TOTAL) ? ext_compute_ready[gb+3] : 0,
               (gb + 3 < HEADS_TOTAL) ? ext_compute_done[gb+3] : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].compute_start : 0,
               (gb + 3 < HEADS_TOTAL) ? op_name(head_ctx_shadow[gb+3].compute_op) : "?",
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].q_started : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].q_compute_done : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].k_started : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].k_compute_done : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].v_started : 0,
               (gb + 3 < HEADS_TOTAL) ? head_ctx_shadow[gb+3].v_compute_done : 0,
  ...
  initial begin
    ...
    $display("cycle grp ret| %-8s rdy done strt op    qst qdn kst kdn vst vdn vld | %-8s rdy done strt op    qst qdn kst kdn vst vdn vld | %-8s rdy done strt op    qst qdn kst kdn vst vdn vld | %-8s rdy done strt op    qst qdn kst kdn vst vdn vld |",
